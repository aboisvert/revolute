...
object Node {
  def apply(o:Any): Node = o match {
    case null => revolute.query.ConstColumn.NULL // FIXME: dependency on revolute.query package
    case n: Node => n.nodeDelegate
    case p: Product => new ProductNode(p)
    case r: AnyRef => throw new QueryException("Cannot narrow "+o+" of type "+r.getClass.getName+" to a Node")
    case _ => throw new QueryException("Cannot narrow "+o+" to a Node")
  }
